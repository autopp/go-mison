...
func TestBuildStructualCharacterBitmaps(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			commaBits:     []string{"00100000000000010000000000000000"},
...
func TestBuildQueriedFieldTable(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			table: queriedFieldTable{
				"abc": &queriedFieldEntry{id: queriedFieldObject, children: queriedFieldTable{"def": &queriedFieldEntry{id: 0}, "ghi": &queriedFieldEntry{id: 1}}},
		...
		},
		{
			queriedFields: []string{`abc\\.def`},
			table: queriedFieldTable{
				`abc\`: &queriedFieldEntry{id: queriedFieldObject, children: queriedFieldTable{"def": &queriedFieldEntry{id: 0}}},
			},
			level: 2,
		},
		{
...
func TestParserState(t *testing.T) {
	cases := []struct {
		...
		{
			json:          []byte(`{"friends": {"id": "1111", "name": "autopp"}}`),
			queriedFields: []string{"friends.id", "friends.name"},
			expected:      []*KeyValue{{0, "1111", `"1111"`, JSONString}, {1, "autopp", `"autopp"`, JSONString}},
		},
		{
